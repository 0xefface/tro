...
namespace RealmOffline.Tcp.Base
{
    ...
    public abstract class ClientBase
    {
        ...
        private delegate void ProcessCommunicationPackets(byte[] packet, AsyncCallback callback);
        //private event ProcessCommunicationPackets AsyncPacketProcessor { get; set; }

        //public CryptoKey server_key { get; set; }
        ...
        protected ClientBase(ServerBase server)
        {
           ...
           // this.AsyncPacketProcessor += new ProcessCommunicationPackets(ProcessPacket);
        ...
        /// </summary>
        /// <param name="num_bytes">The size of the packet in bytes.</param>
        protected abstract void OnReceive(int num_bytes);
        ...
        private static void SendAsyncComplete(object sender, SocketAsyncEventArgs args)
        {
            ...
            try
            {
               // Console.WriteLine("Send of {0} bytes to {1} returns {2}",
                   // args.Count, client.TcpSocket.RemoteEndPoint, args.SocketError);
            }
            catch
            {
        ...
        public virtual void ProcessPacket(byte[] packet, AsyncCallback callback)
        {
            IAsyncResult result = this.BeginPacketProcess(packet);
        }
        private IAsyncResult BeginPacketProcess(byte[] packet)
        {
            return null;
        }
    }
